...
if is_colltrace:
  with open(os.path.join(gensrc, "device_table.cpp"), "w") as f:
    ...
    for fn in primary_funcs:
      fn_no_unroll = (fn.coll, fn.algo, fn.proto, fn.redop, fn.ty, fn.acc, fn.pipeline, fn.reg)
      if fn_no_unroll not in seen_fns:
        reg = fn.reg if fn.reg != "0" else None
        name = paste("_", "ncclDevFunc", fn.coll, fn.algo, fn.proto, fn.redop, fn.ty, fn.acc, fn.pipeline, reg)
        out('   "%s",\n' % name)
