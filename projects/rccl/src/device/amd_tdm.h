...
__device__ inline void loadTileToLds(const void* global, void* lds,
  ...
  asm volatile("s_wait_tensorcnt 10" ::: "memory");
...
// field in the descriptor, so a single op moves up to 65535 bytes. The copy
// path's LDS window is sized to that limit or to remaining per-warp LDS.
template <int cpol>
